...
for(i in clinical_dept_list){
  ...
  ppt_temp <- ppt_temp %>% 
    ph_with(value = "Performance Overview", 
            location = ph_location_label(ph_label = "Title 1")) 
  ppt_temp <- ph_with(ppt_temp, value = fpar(hyperlink_ftext(href = "https://cran.r-project.org/index.html",
  ...
  ppt_temp <- ppt_temp %>%
    add_slide(layout = "Title and Content", master = "Default Theme")
  ppt_temp <- ppt_temp %>% 
    ph_with(value = paste0("Metric Definisions"), 
            location = ph_location_label(ph_label = "Title 1")) 
  ppt_temp <- ph_with(ppt_temp, value = fpar(ftext(slide_num, prop = text_style), fp_p = par_style),
                      location = ph_location_label(ph_label = "Slide Number Placeholder 3"))
  ...
  print(ppt_temp, target = paste0(dir, paste0("FOPR_Report_", gsub("/", "_", i), ".pptx")))
...
for(i in medicine_sub_dept_list){
  ...
  # Export Template ------------------------------------------------------------
  dir <- "~/qpr-template-deck/template-output/"
  print(ppt_temp, target = paste0(dir, paste0("FOPR_Report_DOM_", gsub("/", "_", i), ".pptx")))
